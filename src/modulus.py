...
def decode(inputBytes):
    numArgs = int(int.from_bytes(inputBytes[0:WORD_LENGTH], byteorder='big') / WORD_LENGTH)
    pointers = [int.from_bytes(inputBytes[WORD_LENGTH*i:WORD_LENGTH*(i+1)], byteorder='big') for i in range(numArgs)]
    print(pointers)
...
def encode(outputs):
    ...
    outputBytes = b''
    for i in range(ln):
        outputBytes += runningLength.to_bytes(WORD_LENGTH, byteorder='big')
